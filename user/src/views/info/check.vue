  ...
  <div class="main-box bg">
    ...
    <section  class="base">
      ...
      <div style="padding: 20px 10px">
        <el-button v-if="user.userCheckStatus == 2"  type="success" @click="checkSuccess">审核通过</el-button>
        <el-button v-if="user.userCheckStatus == 2" type="danger" @click="checkShow = true">驳回</el-button>
      </div>
    </section>
    <div class="info-box">
      <el-row :gutter="20">
    ...
    <el-dialog title="不通过" :visible.sync="checkShow">
      <el-form :model="form">
        <el-form-item label="理由" :label-width="formLabelWidth">
          <el-input v-model="form.userCheckMark" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="checkShow = false">取 消</el-button>
        <el-button type="primary" @click="checkBlack">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import { getInfo,getCheckInfo,checkSuccessRequest   } from '@/api/user'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        this.form.userId = id;
        getCheckInfo({userId:id}).then(response => {
      ...
      checkSuccess(){
        this.form.userCheckStatus = 3;
        this.form.userCheckMark = "";
        checkSuccessRequest(this.form).then(response => {
          this.$message('操作成功')
          this.fetchData();
        })
      },
      checkBlack(){
        this.form.userCheckStatus = 4;
        checkSuccessRequest(this.form).then(response => {
          this.$message('操作成功')
